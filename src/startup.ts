...
import {
    createItemsTable,
    createUserEmailsTable,
    createUserTable,
} from './db/create_tables'
...
function createMasterPassword() {
    ...
    while (!passwordIsVerified(pwd)) {
        console.log(
            'That password is not strong enough makes sure it is at least 10 characters long\n'
        )
    ...
    console.log(
        "You're all set! Don't forget your password. Your data will be unrecoverable if you do!"
    )
    return pwd
}
...
async function createNewUser(
    username: string,
    hasMasterPassword: MasterPasswordOpts
) {
...
export async function initApp(): Promise<string> {
    ...
    if (!firstTimeUser) {
        console.log(
            'Hello! It looks like this is your first time using CLI PASS'
        )
        const { username, userMasterPassword } =
            await performFirstTimeUserSetup()
        ...
        if (user.hasMasterPassword === 0) {
            console.log(
                'it looks like you havent yet created a master password'
            )
            user_password = createMasterPassword()
        }
        user_password = createNewFormField(
